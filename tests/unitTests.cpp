...
{
    std::cout << "\n=== Testing Add Customers Functionality ===" << std::endl;
    
    std::string configPath = std::string(ROOT_SOURCE_DIR) + "/config/GameConfig.json";
    Game* game = new Game(configPath);
    game->createNewGame();
    
    game->createCustomerFactories();
    
    EXPECT_NO_THROW({
        game->addCustomers("ignorant", 1);
    }) << "Should be able to add a single customer";
    
    EXPECT_NO_THROW({
        game->addCustomers("average", 2);
    }) << "Should be able to add multiple customers";
    
    EXPECT_NO_THROW({
        game->addCustomers("greenfinger", 1);
    }) << "Should be able to add a greenfinger customer";
    
    vector<Customer*> customers = game->getCustomers();
    EXPECT_GE(customers.size(), 4) << "Should have at least 4 customers";
    
    std::cout << "✓ Added " << customers.size() << " customers successfully" << std::endl;
    
    delete game;
}

TEST(CustomerTests, AddCustomersErrorCases)
{
    std::cout << "\n=== Testing Add Customers Error Cases ===" << std::endl;
    ...
    EXPECT_THROW({
        game->addCustomers("NonExistentType", 1);
    }, std::runtime_error) << "Should throw error for unknown customer type";
    
    EXPECT_THROW({
        game->addCustomers("ignorant", 0);
    }, std::runtime_error) << "Should throw error for zero quantity";
    ...
    std::cout << "✓ Error handling for customer addition works correctly" << std::endl;
    
    delete game;
}

TEST(CustomerTests, AddCustomersWithoutFactories)
{
    std::cout << "\n=== Testing Add Customers Without Factories ===" << std::endl;
    ...
    EXPECT_THROW({
        game->addCustomers("ignorant", 1);
    }, std::runtime_error) << "Should throw error if customer factories are not created";
    
    std::cout << "✓ Correctly throws error when factories are not created" << std::endl;
    
    delete game;
}

TEST(CustomerTests, CustomerFactoryCreation)
{
    std::cout << "\n=== Testing Customer Factory Creation ===" << std::endl;
    ...
    EXPECT_NO_THROW({
        game->createCustomerFactories();
    }) << "Should create customer factories without error";
    
    std::cout << "✓ Customer factories created successfully" << std::endl;
    
    delete game;
}

TEST(CustomerTests, GetAvailableCustomerTypes)
{
    std::cout << "\n=== Testing Get Available Customer Types ===" << std::endl;
    
    std::string configPath = std::string(ROOT_SOURCE_DIR) + "/config/GameConfig.json";
    Game* game = new Game(configPath);
    
    auto customerTypes = game->getAvailableCustomerTypes();
    
    EXPECT_FALSE(customerTypes.empty()) << "Should load customer types from config";
    
    EXPECT_NE(customerTypes.find("ignorant"), customerTypes.end()) << "Should have ignorant customers";
    EXPECT_NE(customerTypes.find("average"), customerTypes.end()) << "Should have average customers";
    EXPECT_NE(customerTypes.find("greenfinger"), customerTypes.end()) << "Should have greenfinger customers";
    
    std::cout << "✓ Loaded customer types from configuration:" << std::endl;
    for (const auto& [type, customers] : customerTypes) {
        std::cout << "  " << type << ": " << customers.size() << " customer(s)" << std::endl;
        for (const auto& customer : customers) {
            std::cout << "    - " << customer.at("name") << std::endl;
        }
    }
